...
\node (start) [startstop, yshift=-2cm, xshift=-1.3cm]{Observations};

\node (in1) [process, right of=start, xshift=1.7cm] {Pick Operators};

\node (dec1) [decision, right of=in1, xshift = 3.5cm] {Learn PSR};

\node (pro2b) [process, below of=dec1, xshift = -3cm] {Query String };
\node (out1) [process, below of=pro2b, xshift=-3cm] {Partition};
\node (stop) [startstop, right of=out1, xshift=4cm] {Probability};

\draw [arrow] (in2) -- node[anchor=west] {   \hspace{0.5cm} \{a,ba,bcacb,... \}} (start);

\draw [arrow] (start) -- node[anchor=north] {} (in1);
\draw [arrow] (in1) -- node[anchor=south] {  \{$A_{a},A_{cb}$ ... \} } (dec1);
\draw [arrow] (dec1) -- node[anchor=east] {abcacb} (pro2b);
\draw [arrow] (pro2b) -- node[anchor=east] {a-bca-cb} (out1);
\draw [arrow] (out1) -- node[anchor=south] {$\alpha_0 A_a A_{bca} A_{cb} \alpha_{\infty}$} (stop);
